...
module.exports = tobz = async (tobz, message) => {
    try {
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            ...
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh stylebot!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 1X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nPeringatan! 2x lagi berkata kasar, stylebot akan kick kamu dari grup!`, id)
                        }
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh stylebot!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
        ...
        function GenerateSerialNumber(mask){
            ...
            return serialNumber;
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin stylebot!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner stylebot!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner stylebot!', id)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 stylebot*`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink stylebot*`, id)
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev stylebot*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev stylebot*`, id)
            }
            break
            case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan stylebot', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
            ...
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin stylebot', id)  
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `6289509938750@c.us`)
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin sytlebot!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin stylebot!`, id)
        ...
        case prefix+'listbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin STYLE BOT!`, id)
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN STYLEBOT
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Stylebot', id)
        ...
        case prefix+'ekickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner stylebot', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin stylebot', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            ...
            await tobz.sendText(from,'stylebot DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'opromote':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin style', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan stylebot`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan stylebot`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan stylebot atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'elainaadmin':
            let admn = `This is list of stylebo Admin\nTotal : ${adminNumber.length}\n`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner stylebot!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin stylebot!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner stylebot!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin stylebot!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner stylebot!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin stylebot!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin stylebot!`, id)
        ...
        case prefix+'bugreport':
            ...
            if(isGroupMsg){
                ...
                tobz.sendText(ownerNumber, `*[BUG REPORT]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me089509938750/${sender.id.match(/\d+/g)}\n\n${bug}`)
                tobz.reply(from, 'Masalah telah di laporkan ke owner BOT, laporan palsu/main2 tidak akan ditanggapi.', id)
            }
            break
         case prefix+'profile':
        ...
        case prefix+'elainagroup':
            tobz.reply(from, `Link Group stylebot :Jangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin stylebot', id)
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup stylebot, Link Ada Di Bio atau Bisa Mengetik #stylebotgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
